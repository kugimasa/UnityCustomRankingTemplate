...
public class RankingRecord　: MonoBehaviour
{
    ...
    [Header("ランキングバッジのサイズ"), SerializeField] private float[] _badgeSize = new float[4];
    ...
    private void SetRankingBadge(int rank)
    {
        if (rank <= 3)
        {
            _rankBadge.GetComponent<RectTransform>().sizeDelta = new Vector2(_badgeSize[rank - 1], _badgeSize[rank - 1]);
            _rankText.gameObject.GetComponent<RectTransform>().sizeDelta = new Vector2(_badgeSize[rank - 1], _badgeSize[rank - 1]);
        ...
        else
        {
            ...
            _rankText.gameObject.GetComponent<RectTransform>().sizeDelta = new Vector2(_badgeSize[3], _badgeSize[3]);
